...
/*! @file
 ...
 @TODO ROSIMAGEを流す。プログラムの整理とコメントの追加
*/
//=================================================//
...
double drawMaxTemp0 = 42;	//表示する最大温度
double drawMinTemp0 = 37;	//表示する最小温度
...
double threshold_Degree_of_Circularity_Lower = 0.7;
...
public:
	...
	cv::Mat M_thermalimg;		// iplimage->cv::Mat用の変数の定義
		...
		: it_(nh_)
	{
		...
		// パブリッシャーの定義
		image_pub_ = it_.advertise("thermal_image_out", 1);
		maxTemp_pub = nh_.advertise<std_msgs::Float32>("max_temp", 100);
	...
	bool processThermalImage()
	{
		...
		thermal_image_out();	// 二値化した画像をROSIMAGEで配信する
...
//=============================================	
	 void imageProcessing()
	 {
		...
		//}
		std_msgs::Float32 maxTemp_msg;
		maxTemp_msg.data = maxTemp;
		maxTemp_pub.publish(maxTemp_msg);
	 }
...
//=============================================
//@name thermal_image_out
//@brief ２値化，ラベリング処理後の画像を配信する．Victimが存在する場合，blobをマークして表示
//@data 2013/11/09
//@attention
//=============================================	
	 void thermal_image_out()
	 {
		// cv_bridge用の変数を定義
		...
		// IplImageをcv::Matに変換してcv_bridge変数に入れる
		M_thermalimg = cv::cvarrToMat(image2);
		//cv_ptr->header   = in_msg->header;
		//cv_ptr->encoding = sensor_msgs::image_encodings::BGR8;
		cv::Mat M_img_out;
		cv::cvtColor(M_thermalimg, M_img_out, CV_RGBA2BGR); // convert from BGR to RGB

		cv::Mat M_img_resize(M_img_out.rows*2/7, M_img_out.cols*2/7, M_img_out.type());
		cv::resize(M_img_out, M_img_resize, M_img_resize.size(), cv::INTER_NEAREST);
		cv_ptr->encoding = sensor_msgs::image_encodings::RGB8;
		//cv_ptr->image    = M_thermalimg;
		cv_ptr->image    = M_img_resize;
...
int main(int argc, char **argv)
{
	...
	if(ros::param::has("/optris/thermo_blob/MaxTemp"))
	{
		ros::param::get("/optris/thermo_blob/MaxTemp", drawMaxTemp0);
	}
	if(ros::param::has("/optris/thermo_blob/MinTemp"))
	{
		ros::param::get("/optris/thermo_blob/MinTemp", drawMinTemp0);
	}
	if(ros::param::has("/optris/thermo_blob/Circularity"))
	{
		ros::param::get("/optris/thermo_blob/Circularity", threshold_Degree_of_Circularity_Lower);
	}
