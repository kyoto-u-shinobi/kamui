...
#define	ROAD_WIDTH	600//650//850		//!< 道の幅[m]//900mm...(JapanOpen),1200mm(WorldCup)

//☆ロボットの筐体，自律走行のパラメータ
#define	ROBOT_WIDTH 350				/*横幅*/
#define	ROBOT_DEPTH 498				/*奥行き*/  
#define	MinF 370//370 //270 //ROAD_WIDTH/2-100,				/*前方の閾値 MinL以上MaxL以下 */        
#define	MaxL 400//350//ROAD_WIDTH/2+100 //450				/*左方の閾値(max)*/   
#define	MinL 340//325  //ROAD_WIDTH/3, //-50,//300						//600 左方の閾値(min)
//#define	SMaxL ROAD_WIDTH/2 + 50  //400 		/*左方の閾値(smax)*/  
#define	TRANSLATIONAL_VEL 28//14 //10,//7,//10,//25//32			//並進速度(kamuiへの指令値になっている+-100の範囲)    弄ります by rocket
#define	ROTATIONAL_VEL 28//14//10//7,//15				//角速度(kamuiへの指令値になっている+-100の範囲)
...
void cloudCallback(const sensor_msgs::PointCloud::ConstPtr& msg)
{
	...
	for(int i = 0; i < (int)msg->points.size(); i++){
		...
		if(urg_x >= 100){
			...
			//if( (urg_y > -y_RANGE) && (urg_y < +y_RANGE) )
			{
				if(urg_x < dist.front)
				{
...
{
	if(dist.left>ROAD_WIDTH/2+200){//右壁が遠いと仮想の壁を作成
