...
// ジンバルのLRFデータから計算
void cloudCallback2(const sensor_msgs::PointCloud::ConstPtr& msg)
{
    double urg_x, urg_y;
    wall_follower::walls walls;
    ...
    for(int i = 0; i < (int)msg->points.size(); i++){
        ...
        if(urg_x >= 100){
            ...
            if( (urg_y > -(ROBOT_WIDTH/2+10)) && (urg_y < +(ROBOT_WIDTH/2+10)) )
            {
                if(urg_x < dist2.front)
                {
                    dist2.front = (int)urg_x;
                }
            }
            if(urg_x <= ROAD_WIDTH/2){
                //右前の方向
                if ((-urg_y > 21) && (-urg_y < dist2.right) )
                {
                    dist2.right = -(int)urg_y;
                }
                //左前の方向
                if( (urg_y > 21) && (urg_y < dist2.left))
                {
                    dist2.left = (int)urg_y;
                }
            }
        }
        else if(urg_x < 100 && urg_x >= -ROBOT_WIDTH/2+100)//後方のデータ
        {
            //右後ろの方向
            if ( (-urg_y > 21) && (-urg_y < dist2.right2))
            {
                dist2.right2 = -(int)urg_y;
            }
            //左後ろの方向
            if ( (urg_y > 21) && (urg_y < dist2.left2))
            {
                dist2.left2 = (int)urg_y;
            }
        }
    }
    //ROS_INFO("F=%4.0f  L=%4.0f  L2=%4.0f  R=%4.0f  R2=%4.0f", dist2.front,  dist2.left, dist2.left2, dist2.right, dist2.right2);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber urg_sub2 = n.subscribe("gimbal/cloud", 1000, cloudCallback2);
